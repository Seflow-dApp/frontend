(function () {
  ...
  function isTextEditable(element) {
    ...
    return textTags.includes(tag) && element.children.length === 0 && element.childNodes.length > 0;
  }
      ...
      if (element) {
        if (e.data.action === "style") {
          ...
          const getClassesToRemove = (incoming) => {
            ...
            const sideKey =
              group === "paddingSide" || group === "marginSide" ? cls.substring(0, 2) : null;
            ...
            return (c) => {
              ...
              switch (group) {
                case "textColor":
                  return matchers.textColor.test(token);
                case "textSize":
                  return matchers.textSize.test(token);
                case "bgColor":
                  return matchers.bgColor.test(token);
                case "borderColor":
                  return matchers.borderColor.test(token);
                case "borderWidth":
                  return matchers.borderWidth.test(token);
                case "rounded":
                  return matchers.rounded.test(token);
                case "fontWeight":
                  return matchers.fontWeight.test(token);
                case "paddingAll":
                  return matchers.paddingAll.test(token);
                case "paddingSide":
                  return new RegExp("^" + sideKey + "-\\d+$").test(token);
                case "marginAll":
                  return matchers.marginAll.test(token);
                case "marginSide":
                  return new RegExp("^" + sideKey + "-\\d+$").test(token);
                default:
                  return false;
          ...
          if (spacingMatch) {
            ...
            const setSpacing = (prop) => {
              element.style[prop] = value;
            };
            if (kind === "p") {
              if (!side) {
                setSpacing("paddingTop");
                setSpacing("paddingRight");
                setSpacing("paddingBottom");
                setSpacing("paddingLeft");
              }
              ...
              if (!side) {
                setSpacing("marginTop");
                setSpacing("marginRight");
                setSpacing("marginBottom");
                setSpacing("marginLeft");
              }
          ...
          const textSizeMatch = newClass.match(
          ...
          const fontWeightMatch = newClass.match(
